...
/**
 * Test the database connection by querying the migrations table.
 */
async function actionTest() {
  ...
  try {
    ...
    throw new Error('test migration connection failed: ' + err.message);
  }
}
...
async function actionDown({ rows }) {
  ...
  wrapTransaction('migrate up', async () => {
...
async function wrapTransaction(actionName, asyncFn) {
  console.log('begin transaction: ' + actionName);
  try {
    await sql.query('BEGIN');
    await asyncFn();
    await sql.query('COMMIT');
    console.log('commit transaction: ' + actionName);
  } catch (e) {
    console.log('error: ' + e.message);
    console.log(` ${actionName} failed -- rollback`);
    await sql.query('ROLLBACK');
    new Error(`${actionName} failed:` + e.message);
  }
}
...
async function runCLIAction() {
  ...
  console.log(`Migrate ${direction} done`);
  process.exit(0);
}
