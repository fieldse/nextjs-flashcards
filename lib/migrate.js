...
async function actionTest() {
  ...
  try {
    const rows = await sql`select id, name, created_at from migrations order by id asc`;
    if (rows.length) {
      console.log(
        `current migration status:` + rows.map((r, index) => `${index} - ${r.name}`).join('\n '),
      );
    }
  } catch (err) {
...
async function actionDown({ rows }) {
  ...
  try {
    ...
    await sql.query('ROLLBACK');
    new Error(`migrate down failed: ${filename}` + e.message);
  }
}
